...
double shuntingYardEvaluator(arx::deque<Token> equation, bool isRadians = false) { // main function for evaluating and solving equation once it is in reverse polish notation
  ...
  while (stack.size() > 0 && stack.size() < 10000) { // keep looping while input stack is not empty, and while the input stack hasn't overflowed
    ...
    if (token.type == NUMBER) { // check if token is a number
      ...
      if (token.type == OPERATOR) { // check if the token is an operator
        hist.pop_back(); // remove 2nd last item from number history if an operator is being used
      }

      Serial.println(token.math(x, y, isRadians)); // evaluate math operation/function and assign it to a variable
      hist.push_back(token.math(x, y, isRadians)); // add output to the number history
...
void loop() { // loop code to run indefinitely
  ...
  delay(50); // delay of 50ms to keep the code running in a stable fashion
