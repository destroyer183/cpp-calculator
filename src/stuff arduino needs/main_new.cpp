...
arx::deque<Token> shuntingYardConverter(String equation) {
    ...
    // for (String i: inStack) {std::cout << i;}
    for (String i: inStack) {
        Serial.print(i);
    }
...
arx::vector<double> findNumbers(tokenType type, arx::deque<double> hist) {
    ...
    if (type == FUNCTION) {
        return arx::vector<double> {hist.at(hist.size() - 1), 0.0};
    }

    // return two numbers from the end of the number history if it is for an operation
    else if (type == OPERATOR) {
        return arx::vector<double> {hist.at(hist.size() - 2), hist.back()};
    }
...
double shuntingYardEvaluator(arx::deque<Token> equation, bool isRadians) {
...
void main() {
    ...
    Serial.print("test: ");
    Serial.println(token.math(60, 0, isRadians));
    ...
    arx::deque<Token> stack = shuntingYardConverter(equation);
